...
class FoodRecommender:
    ...
    def too_sweet(self, food_id):
        s = DBHelper.conn.execute("SELECT ifnull(avg(sweet),-1) FROM comments WHERE food_id=?", (food_id, )).fetchone()[0]
        if s == -1:
    ...
    def too_sour(self, food_id):
        s = DBHelper.conn.execute("SELECT ifnull(avg(sour),-1) FROM comments WHERE food_id=?", (food_id, )).fetchone()[0]
        if s == -1:
            self.except_list.append(str(food_id))
        else:
            self.sour_ub = s-0.1
    ...
    def too_oily(self, food_id):
        s = DBHelper.conn.execute("SELECT ifnull(avg(oily),-1) FROM comments WHERE food_id=?", (food_id, )).fetchone()[0]
        if s == -1:
            self.except_list.append(str(food_id))
        else:
            self.oily_ub = s-0.1
    ...
    def too_salty(self, food_id):
        s = DBHelper.conn.execute("SELECT ifnull(avg(salty),-1) FROM comments WHERE food_id=?", (food_id, )).fetchone()[0]
        if s == -1:
            self.except_list.append(str(food_id))
        else:
            self.salty_ub = s-0.1
